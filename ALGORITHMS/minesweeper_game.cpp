...
bool playminesun l(char myboard[][max_side],char realboard[][max_side],int mines[][2],int row,int 
col,int *moves_le ) 
{ 
    if(myboard[row][col]!='-') 
        return false; 
        ...
        { 
            ...
            { 
                if(ismine(row-1,col,realboard)==false) 
                    playminesun l(myboard, realboard, mines, row-1, col, moves_le ); 
            } 
            if (isvalid (row+1, col) == true) 
            { 
                if (ismine (row+1, col, realboard) == false) 
            ...
            { 
                if (ismine (row, col+1, realboard) == false) 
                    playminesun l(myboard, realboard, mines, row, col+1, moves_le ); 
            } 
            if (isvalid (row, col-1) == true) 
            { 
                if (ismine (row, col-1, realboard) == false) 
            ...
            { 
                if (ismine (row-1, col+1, realboard) == false) 
                    playminesun l(myboard, realboard, mines, row-1, col+1, moves_le ); 
            } 
            if (isvalid (row-1, col-1) == true) 
            { 
                if (ismine (row-1, col-1, realboard) == false) 
...
void ini alise(char realboard[][max_side],char myboard[][max_side]) 
{ 
    srand( me(NULL)); 
...
{ 
    ...
    int moves_le =SIDE*SIDE-MINES; 
    int x,y; 
    int mines[max_mine][2]; 
 
    ini alise(realboard,myboard); 
    ...
    { 
        ...
        gameover = playminesun l(myboard,realboard,mines,x,y,&moves_le ); 
 
        if((gameover==false)&&(moves_le ==0)) 
        { 
            cout<<"\nYou won !\n"; 
            gameover=true; 
        } 
